...
app.layout = html.Div(
    [
        ...
        dbc.InputGroup(
            [
                dbc.InputGroupText('Error Correction'),
                dbc.Select(
                    id='error-level',
                    options=[
                        {'value': 'L', 'label': 'L (Min)'},
        ...
        html.Div(id='my-output', style={'text-align': 'center', 'margin': 'auto'}),
    ]
)
...
@callback(
    ...
    Input('scale', 'value'),
    Input('error-level', 'value'),
    Input('go', 'n_clicks'),
)
def update_code(content, img_type, scale, error, _):
    if not content:
        return ''

    _qrcode = segno.make_qr(content, error=error)
    if img_type == 'svg':
